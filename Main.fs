...
let downAction aList =
    List.transpose ((leftAction ((List.transpose aList) |> List.map List.rev)) |> List.map List.rev)
...
let createBoard (board: int list list) =
    ...
    |> List.map (fun x ->
        ...
        |> List.map (fun y ->
            ...
            let num =
                if y = 0 then
                    (if r.NextDouble() < 0.75 then 0
                     elif r.NextDouble() < 0.9 then 2
                     else 4)
                else
                    y
            num))

let padBoth(src: string, length: int) =
    let spaces = length - src.Length
    let padLeft = spaces/2 + src.Length
    src.PadLeft(padLeft).PadRight(length)


let printBoard board =
    printfn "SCORE: %i " score
    (List.replicate 5 "_______") |> Seq.iter (fun x -> printf "%s " x)
    printfn "\n"
    for line in board do
        for element in line do
            let str = padBoth(element.ToString(), 6)
            printf "%s║"  str
        printf "\n"
...
let moveBoard key =
    match key with
    | ConsoleKey.UpArrow -> x <- upAction x
    | ConsoleKey.DownArrow -> x <- downAction x
    | ConsoleKey.LeftArrow -> x <- leftAction x
    | ConsoleKey.RightArrow -> x <- rightAction x
    | _ ->
        x <- board
        score <- 0
    x <- createBoard x
